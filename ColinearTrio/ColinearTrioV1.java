...
public class ColinearTrioV1 {
	 ...
	 * @param thePoints
	 */
	public ColinearTrioV1(Point[] thePoints) {
		 myPoints = thePoints;
	...
	Point[] testPoints = generateRandomPointArray(200);
	//Point[] testPoints = generateKnownColinear(50); // more than 50 takes a long time.
		...
		ColinearTrioV1 ct = new ColinearTrioV1(testPoints); //------------------------------------
	...
	private void findTrios() {
		...
		for(int i = 0; i < myPairs.size(); i++) {
			...
			for(int j = 0; j < myPoints.length; j++) {
				...
				if(!myPairs.get(i).isDuplicatePoint(myPoints[j]) && myPairs.get(i).isColinear(myPoints[j]))  {

					if(DEBUG  && ++counter % mod == 0) System.out.print("trio ");
					
					if (USE_LIST) {
						// If this line data already exists, then get its existing quantity.
						int quantity = (map.containsKey(lineData)) ? map.get(lineData) : 0;
	 ...
	 */
  	private class LineData {
		...
		public boolean equals(Object o) {
			return ((LineData)o).yIntercept == this.yIntercept && ((LineData)o).slope == this.slope;
		}
	...
	private class PointPair {
		...
		public PointPair(final Point pA, final Point pB) {
			...
			// m = (y2 - y1) / (x2 - x1) 
			lineData.slope = ((pointB.getY() - pointA.getY()) / (pointB.getX() - pointA.getX())) + 0.0;
